@extends('revenue-inspector.deathSection.layouts.death_app')
...
@section('content')
                    ...
                    <form>
                        <div class="card mb-4 custom-form-card">
                            @forelse ($deadpersondetails as $deadperson)
                                @php
                                    $user = Auth::user();
                                    $roleName = $user->role_name ?? ($user->role->name ?? '');
                                    $isRI = $roleName === 'Revenue Inspector';
                                    $riStatus = $deadperson->approved_rejected_by_ri; // 1 = approved, 2 = rejected
                                ...
                                <div class="card-body border rounded mb-3">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center flex-wrap mt-2">
                                        {{-- LEFT: Always show application status --}}
                                        <div>
                                            @if ($deadperson->application_status == 'approved')
                                                <span class="badge bg-success text-white">
                                                    <i class="fas fa-check-circle me-1"></i> Approved
                                                </span>
                                            @elseif($deadperson->application_status == 'pending')
                                            ...
                                            @else
                                                <span class="badge bg-danger text-white">
                                                    ...
                                                    {{ ucfirst($deadperson->application_status) }}
                                                </span>
                                            @endif
                                        </div>

                                        {{-- RIGHT: Approve/Reject or Label + View Button --}}
                                        <div class="d-flex align-items-center gap-2 mt-2 mt-md-0">
                                            @if ($riStatus === 1)
                                                <span class="badge bg-success">
                                                    {{ $isRI ? 'Approved by You' : 'Approved by Revenue Inspector' }}
                                                </span>
                                            @elseif ($riStatus === 2)
                                                <span class="badge bg-danger">
                                                    {{ $isRI ? 'Rejected by You' : 'Rejected by Revenue Inspector' }}
                                                </span>
                                            @elseif ($isRI)
                                                <button type="button" class="btn btn-success btn-sm open-remark-modal"
                                            ...
                                            {{-- View Details Button --}}
                                            <button type="button" class="btn btn-outline-primary btn-sm open-modal"
    ...
    <div class="modal fade" id="remarkModal" tabindex="-1" aria-labelledby="remarkModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('rinspactor.update.application.status') }}" method="POST">
                @csrf
