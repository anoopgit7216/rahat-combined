...
namespace App\Http\Controllers\NaibTahsildar;
...
class NaibTahsildarController extends Controller
{
    public function index()
    {
        return view('naib-tahsildar.naib_dashboard');
    }
    public function ahaitukDashboard()
    {
        return view('naib-tahsildar.ahaituk_dashboard');
    }
    public function revanueInspectorDashboard()
    {
        ...
        return view('naib-tahsildar.deathSection.death_dashboard', compact('deadpersoncountapproved', 'deadpersoncountdelayed', 'deadpersonscount', 'deadpersoncountpending'));
    }
  ...
  public function allDeathApplications()
{
    ...
    $query = DB::table('dead_person as dead')
        ...
        ->whereIn('dead.approved_rejected_by_naibtahsildar', [0, 1, 2])
        ->orderByDesc('dead.id');

    $deadpersondetails = $query->get();

    return view('naib-tahsildar.deathSection.applications', compact('deadpersondetails', 'roleName'));
}


    public function updateStatusByNtahsildar(Request $request)
    {
        $request->validate([
