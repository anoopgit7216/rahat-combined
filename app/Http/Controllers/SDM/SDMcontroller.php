...
class SDMcontroller extends Controller
{
    ...
    {
        return view('sdm.tahsildar_dashboard');
    }
    public function ahaitukDashboard()
    {
        return view('sdm.ahaituk_dashboard');
    }
    public function revanueInspectorDashboard()
    {
        ...
        return view('sdm.deathSection.death_dashboard', compact('deadpersoncountapproved', 'deadpersoncountdelayed', 'deadpersonscount', 'deadpersoncountpending'));
    }
    ...
    public function allDeathApplications()
    {
        ...
        $query = DB::table('dead_person as dead')
            ...
            ->whereIn('dead.approved_rejected_by_sdm', [0, 1, 2])
            ->orderByDesc('dead.id');

        $deadpersondetails = $query->get();

        return view('sdm.deathSection.applications', compact('deadpersondetails', 'roleName'));
    }

    public function updateStatusBysmagistrate(Request $request)
    {
        $request->validate([
