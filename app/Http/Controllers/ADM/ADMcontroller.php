...
class ADMcontroller extends Controller
{
    ...
    {
        return view('adm.tahsildar_dashboard');
    }
    public function ahaitukDashboard()
    {
        return view('adm.ahaituk_dashboard');
    }
    public function revanueInspectorDashboard()
    {
        ...
        return view('adm.deathSection.death_dashboard', compact('deadpersoncountapproved', 'deadpersoncountdelayed', 'deadpersonscount', 'deadpersoncountpending'));
    }
    ...
    public function allDeathApplications()
    {
        ...
        $query = DB::table('dead_person as dead')
            ...
            ->where('dead.approved_rejected_by_tahsildar', 1)
            ->where('dead.approved_rejected_by_sdm', 1)
            ->whereIn('dead.approved_rejected_by_adm', [0, 1, 2])
            ->orderByDesc('dead.id');

        $deadpersondetails = $query->get();

        return view('adm.deathSection.applications', compact('deadpersondetails', 'roleName'));
    }


    public function updateStatusBydmagistrate(Request $request)
    {
        $request->validate([
